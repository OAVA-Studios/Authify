...
async function executeInIsolate(sourceCode, input, timeout, memoryMb) {
    ...
    try {
        ...
        await jail.set('__log', new ivm.Reference((...args) => {
            logs.push(args.map((a) => (typeof a === 'object' ? JSON.stringify(a) : String(a))).join(' '));
        }));
        await jail.set('__fetch', new ivm.Reference(async (...args) => {
...
export function startWorker() {
    const worker = new Worker('authify:functions', async (job) => {
        const data = job.data;
        ...
        const output = await executeInIsolate(data.sourceCode, input, data.timeout, data.memory);
        await db
